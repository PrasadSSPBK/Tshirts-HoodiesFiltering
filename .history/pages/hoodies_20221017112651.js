...
export default function Hoodies({ data }) {
   ...
   const minToMax=()=>{
let minToMaxProducts=products.filter
   }
    ...
    return (
        ...
        <section className="text-gray-600 body-font" >
            <button type="button" className="bg-black text-white" onClick={minToMax}>MINTOMAX</button>
                                        ...
                                        {item.size.map((eachSize) => {
                                            return (
                                                <div key={item.id} className="flex">
                                                    ...
                                                    <div className=" border bg-black border-black  m-1 p-2">
                                                        <p className="text-green-600 text-sm">{eachSize}</p>
...
export async function getServerSideProps() {
    ...
    const response = await fetch("http://localhost:4000/tshirts?category=hoodies")
