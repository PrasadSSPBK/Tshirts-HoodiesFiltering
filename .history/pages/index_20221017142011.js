            ...
            {(categoryArray).map((item) => {
              return (
                <div key={item} className="flex mx-5 my-4 px-2">
                  <input type="checkbox" value={item} id="cost" className="font-semibold mx-2 text-red-600" onChange={handleOnChangeCategory} />
                  <label htmlFor="cost">{item.toUpperCase()}</label>
                </div>
              )
            })}
          </div>
