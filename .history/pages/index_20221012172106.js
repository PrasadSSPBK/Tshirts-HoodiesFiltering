...
export default function Home({data}) {
 const [products,setProducts]=useState(data)
  ...
  const handleOnChange=(e)=>{
    ...
    let selectedPrice=price.join(",")
    console.log(selectedPrice)
    {selectedPrice.map((eachPrice)=>{
      return(

        filterdProducts = products.filter((each) => (
  each.price==eachPrice
    ))
      )
    })}
...
//   let selectedPrice=price.join(",")
//  const response1= await fetch(`http://localhost:4000/tshirts?price=${price}`)
