      ...
      <div className="container flex px-5 py-24 mx-auto w-full">
        <div className="w-1/4 bg-gray-400 mx-5 shadow-2xl">
          {Object.keys(data).map((item)=>{
            return(
              <div key={data[item].id} className="flex mx-5 my-4 px-2">
                <input type="checkbox" value={data[item].price} id="cost" onChange={handleOnChange}/>
                <span htmlFor="cost ">₹{data[item].price}</span>
              </div>
            )
          })}
        </div>
